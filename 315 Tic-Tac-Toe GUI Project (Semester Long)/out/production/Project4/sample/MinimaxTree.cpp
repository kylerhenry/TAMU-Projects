...
MinimaxTree::MinimaxTree(vector<vector<vector<char>>> b, char piece) : computerPiece(piece) {
	...
	for (int k = 0; k < 4; k++) {
		for (int j = 0; j < 4; j++) {
			for (int i = 0; i < 4; i++) {
				Node* child = new Node_AI(root, 'X', computerPiece, b, { i, j, k }); //X always moves first
			...
			for (int j = 0; j < 4; j++) {
				for (int i = 0; i < 4; i++) {
					...
					if (child->getBoard()[i][j][k] == ' ') {
						child->insertChild(new Node_AI(child, 'O', computerPiece, child->getBoard(), { i , j, k }));
					}
				}
			}
		}
...
void  MinimaxTree::update(char piece, vector<int> lastMove) {
	...
	for (unsigned int i = 0; i < root->getChildren().size(); i++) {
		Node_AI* child = root->getChildren()[i];
